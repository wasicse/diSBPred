...
int main(int argc, char **argv)
{
  ...
  if ((path=getenv("IUPred_PATH"))==NULL) {
    ...
    path="/home/presentation/Sumaiya/DISORDER_PROTEIN_PREDICTION/DisPredict_v1.0/Software/AdditionalFiles/iupred/";
  } 
...
void *my_malloc(size_t size)
{
  ...
  new_mem = malloc(size);
  if (new_mem == NULL) {
    fprintf(stderr, "can't allocate enough memory: %zd bytes\n", size);
  }
  return new_mem;
}
